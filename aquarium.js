...
AQ.FieldType.record_methods.can_produce = function(fv) {
  ...
  if (ft.ftype === 'sample' && fv.field_type.ftype === 'sample') {
    aq.each(ft.allowable_field_types, (aft) => {
      if ( !fv.aft ) {
        if (!fv.corrupt ) {
          add_designer_message(`Warning. field value '${fv.name}' has no allowable types. This plan may have been corrupted due to altered i/o definitions.`);
          fv.corrupt = true;
        }
...
AQ.Operation.record_methods.valid = function() {
  ...
  operation.field_values.forEach(field_value => {
    if ( !field_value.field_type.array &&
          field_value.child_sample_id ) {
      valid = valid && operation.routing[field_value.routing] == field_value.sid;
    }
  })
